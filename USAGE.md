...
2. **校正階段（4 回合，約 15 秒）**：
   ...
   ESP32 開機後會進入自動校正模式，請依照 LED 指示操作：

   | 時間     | LED 狀態                  | 你應該做什麼                  |
   | -------- | ------------------------- | ----------------------------- |
   | 0-3 秒   | 🔵 **慢閃**（1 秒週期）   | 準備站好，等待開始            |
   | 3-6 秒   | ⚫ **熄滅**               | 保持站立不動（第 1 次站立）   |
   | 6-9 秒   | 🔵 **快閃**（500ms 週期） | 抬起膝蓋至水平（第 1 次抬腳） |
   | 9-12 秒  | ⚫ **熄滅**               | 保持站立不動（第 2 次站立）   |
   | 12-15 秒 | 🔵 **快閃**（500ms 週期） | 抬起膝蓋至水平（第 2 次抬腳） |
   | 15 秒+   | ⚫ **熄滅**               | ✅ 校正完成！可開始測試       |

   > 💡 **觀察技巧**：你會看到 **3 次「閃爍 → 熄滅」的轉換**，這是正常的：
   >
   > - 第 1 次：準備階段結束（慢閃 → 熄滅）
   > - 第 2 次：第 1 次抬腳結束（快閃 → 熄滅）
   > - 第 3 次：第 2 次抬腳結束（快閃 → 熄滅）= 校正完成
...
| ΔY(cm)       | 相對前後位移    | 公分  |
| ΔZ(cm)       | 相對上下位移    | 公分  |
| 絕對 X(cm)   | 膝蓋絕對 X 座標 | 公分  |
| 絕對 Y(cm)   | 膝蓋絕對 Y 座標 | 公分  |
| 絕對 Z(cm)   | 膝蓋絕對 Z 座標 | 公分  |
| 是否穩定     | 數值穩定度      | 是/否 |
| 加速度 X/Y/Z | 三軸加速度      | g     |
| 陀螺儀 X/Y/Z | 三軸角速度      | deg/s |
...
| 看到什麼                         | 代表什麼 | 你該怎麼做         |
| -------------------------------- | -------- | ------------------ |
| LED 慢閃（約 1 秒亮 1 秒滅）     | 準備階段 | 站好不動，等待開始 |
| LED 熄滅                         | 站立階段 | 維持站姿不動       |
| LED 快閃（約 0.5 秒亮 0.5 秒滅） | 抬腳階段 | 將膝蓋抬至水平     |
| LED 一直熄滅不再閃               | 校正完成 | 可以開始跑步測試   |

**正常流程**：你會看到 3 次「閃爍 → 熄滅」的轉換，總共約 15 秒。

---

### **問題 2：ESP32 無法連接 Wi-Fi**
